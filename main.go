...
func main() {
	...
	// for i := 0; i < len(programData); i += 12 {
	// 	instr := new(Instruction)

	// 	copy(instr.INSTR[:], programData[i:i+2])
	// 	copy(instr.X[:], programData[i+2:i+4])
	// 	copy(instr.Y[:], programData[i+4:i+6])
	// 	copy(instr.N[:], programData[i+6:i+8])
	// 	copy(instr.NN[:], programData[i+8:i+10])
	// 	copy(instr.NNN[:], programData[i+10:i+12])
